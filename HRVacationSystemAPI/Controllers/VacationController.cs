...
namespace HRVacationSystemAPI.Controllers
{
    ...
    public class VacationController : ApiController
    {
        ...
        [System.Web.Http.HttpGet]
        [System.Web.Http.Route("izin_kontrol")]
        public APIResponse<PersonelVacation> GetPersonelVacation(string email)
        {
            try
            {
                var personel = personelManager.GetPersonelProile(email);
                ...
                {
                    var result = personelVacationManager.GetPersonelVacation(personel.Id);

                    if (result == null)
                        return new APIResponse<PersonelVacation>()
                        {
                            IsSuccess = false,
                            Message = $"{personel.Name} isimli personelin izin talebi bulunmamaktadır!",
                            Data = new PersonelVacation()
                        };
                    else
                        return new APIResponse<PersonelVacation>()
                        {
                            IsSuccess = true,
                            Message = $"{personel.Name} isimli personelin izin talebi vardır!",
                            Data = new PersonelVacation()
                            {
                                Id = result.Id,
                                CreatedDate = result.CreatedDate,
                                PersonelId = result.Id,
                                VacationTypeId = result.VacationTypeId,
                                VacationStartDate = result.VacationStartDate,
